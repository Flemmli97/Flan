...
public class ClaimDisplay {
    ...
    public ClaimDisplay(Claim claim, EnumDisplayType type, int y) {
        ...
        this.type = type;
        this.height = Math.max(1 + claim.getWorld().getBottomY(), y);
        switch (type) {
            case SUB -> {
    ...
    private static int[] nextAirAndWaterBlockFrom(WorldChunk chunk, int x, int y, int z) {
        ...
        if (state.getMaterial().isReplaceable()) {
            ...
            while (state.getMaterial().isReplaceable() && !chunk.isOutOfHeightLimit(pos)) {
                pos.move(0, -1, 0);
            ...
            if (startedInLiquid) {
                ...
                state = chunk.getBlockState(pos);
                while (state.getMaterial().isLiquid() && !chunk.isOutOfHeightLimit(pos)) {
                    pos.move(0, 1, 0);
                    state = chunk.getBlockState(pos);
                }
                if (state.getMaterial().isReplaceable())
