        ...
        if (mode.isSubclaim) {
            ...
            if (sub != null) {
                if (sub.canInteract(player, perm, pos))
                    return sub;
                player.sendSystemMessage(ClaimUtils.translatedText("flan.noPermission", ChatFormatting.DARK_RED));
                return null;
            }
            if (!fallbackMain) {
                player.sendSystemMessage(ClaimUtils.translatedText("flan.noSubClaim", ChatFormatting.RED));
                return null;
            }
        }
