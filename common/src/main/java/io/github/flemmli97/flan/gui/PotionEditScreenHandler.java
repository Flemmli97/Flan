...
public class PotionEditScreenHandler extends PagedServerOnlyScreenHandler<Claim> {
    ...
    protected void fillInventoryWith() {
        Map<MobEffect, Integer> potions = this.data.getPotions();
        List<MobEffect> key = Lists.newArrayList(potions.keySet());
        key.sort(Comparator.comparing(eff -> Registry.MOB_EFFECT.getKey(eff).toString()));
        for (int i = 0; i < 54; i++) {
            if (i == 0) {
                ...
                if (id < potions.size()) {
                    ...
                    effectStack.getOrCreateTag().putString("FlanEffect", Registry.MOB_EFFECT.getKey(effect).toString());
                    effectStack.getTag().putInt("CustomPotionColor", PotionUtils.getColor(inst));
                    txt.append(new TextComponent("-" + potions.get(effect)));
    ...
    protected boolean handleSlotClicked(ServerPlayer player, int index, Slot slot, int clickType) {
        ...
        if (index == 3) {
            ...
            player.getServer().execute(() -> StringResultScreenHandler.createNewStringResult(player, (s) -> {
                ...
                int amp = 1;
                MobEffect effect = Registry.MOB_EFFECT.get(new ResourceLocation(potion[0]));
                ...
                this.data.addPotion(effect, amp);
                player.closeContainer();
        ...
        if (!stack.isEmpty() && this.removeMode) {
            ...
            this.data.removePotion(Registry.MOB_EFFECT.get(new ResourceLocation(effect)));
