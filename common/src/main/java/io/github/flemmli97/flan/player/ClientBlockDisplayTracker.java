...
public class ClientBlockDisplayTracker {
    ...
    public void displayFakeBlocks(UUID id, Set<DisplayData> blocks) {
        ...
        if (current != null) {
            ...
            diff.forEach(d -> this.player.connection.send(new ClientboundBlockUpdatePacket(d.pos, this.player.getLevel().getBlockState(d.pos))));
        }
    ...
    public void resetFakeBlocks(UUID id) {
        ...
        if (current != null) {
            current.forEach(d -> {
                ...
                this.player.connection.send(new ClientboundBlockUpdatePacket(d.pos, state == null ? this.player.getLevel().getBlockState(d.pos) : state));
            });
        }
    }
