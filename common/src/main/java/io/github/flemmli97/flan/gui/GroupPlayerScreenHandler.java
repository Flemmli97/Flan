...
public class GroupPlayerScreenHandler extends PagedServerOnlyScreenHandler<ClaimGroup> {
    ...
    protected void fillInventoryWith() {
        List<String> players = this.data.getClaim().playersFromGroup(this.player.getServer(), this.data.getGroup());
        for (int i = 0; i < 54; i++) {
            if (i == 0) {
    ...
    protected boolean handleSlotClicked(ServerPlayer player, int index, Slot slot, int clickType) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (this.removeMode && tag.contains("Id")) {
                this.data.getClaim().setPlayerGroup(tag.getUUID("Id"), null, false);
