...
public class CommandHelp {
    ...
    public static int helpCmd(CommandContext<CommandSourceStack> context, String command) {
        ...
        for (int i = 0; i < cmdHelp.length; i++) {
            if (i == 0) {
                context.getSource().sendSuccess(PermHelper.simpleColoredText(String.format(ConfigHandler.LANG_MANAGER.get("helpCmdSyntax"), cmdHelp[i]), ChatFormatting.GOLD), false);
