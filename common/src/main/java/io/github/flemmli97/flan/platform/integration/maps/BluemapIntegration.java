...
import static io.github.flemmli97.flan.platform.integration.maps.MapUtils.fillColor;
        ...
        BlueMapAPI.getInstance().flatMap(api -> api.getWorld(claim.getLevel())).ifPresent(world -> {
            for (BlueMapMap map : world.getMaps()) {
                ...
                ExtrudeMarker marker = ExtrudeMarker.builder()
                        .label(MapUtils.claimLabel(claim))
                        .depthTestEnabled(false)
                        // Seems you need the offset
                        .shape(Shape.createRect(dim.minX(), dim.minZ(), dim.maxX() + 1, dim.maxZ() + 1), dim.minY(), dim.maxY())
                        .lineColor(new Color(MapUtils.lineColor(claim.isAdminClaim()), 0.8F))
                        .lineWidth(3)
                        .fillColor(new Color(MapUtils.fillColor(claim.isAdminClaim()), 0.2F))
                        .build();
                ...
                updateMarkers(map, markerSet -> {
                    markerSet.getMarkers().forEach((id, marker) -> {
                        ...
                        if (claim != null && marker instanceof ShapeMarker shapeMarker) {
                            shapeMarker.setLineColor(new Color(MapUtils.lineColor(claim.isAdminClaim()), 0.8F));
